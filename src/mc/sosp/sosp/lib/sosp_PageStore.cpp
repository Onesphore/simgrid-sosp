...
//TODO:
// -change the name of these constants
// - use 'sysconf()' to get the page size
const size_t xbt_pagesize = 4096;
const size_t xbt_pagebits = 12;
 ...
 */
static inline PageStore::hash_type mc_hash_page(const void* data)
{
...
PageStore::PageStore(size_t size) : memory_(nullptr), capacity_(size), top_index_(0)
{
  ...
  if (memory == MAP_FAILED)
    handle_error("Could not mmap initial snapshot pages.");
...
void PageStore::resize(std::size_t size)
{
  ...
  if (new_memory == MAP_FAILED)
    handle_error("Could not mremap snapshot pages.");
#else
...
{
  // TODO: we can use BOOT assert, it's cleaner
  assert(top_index_ <= this->capacity_ && "top_index is not consistent");
  ...
  assert(this->page_counts_[pageno] == 0 && "Allocated page is already used");
