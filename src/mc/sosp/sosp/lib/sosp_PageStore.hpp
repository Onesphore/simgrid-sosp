...
#ifndef SOSP_PAGESTORE_HPP
#define SOSP_PAGESTORE_HPP
...
// used 'inline' instead of 'XBT_ALWAYS_INLINE'
inline void PageStore::unref_page(std::size_t pageno)
{
  if ((--this->page_counts_[pageno]) == 0)
    this->remove_page(pageno);
}

inline void PageStore::ref_page(size_t pageno)
{
  ++this->page_counts_[pageno];
}

// TODO: implement mmu::join
// inline const void* PageStore::get_page(std::size_t pageno) const
// {
//   return (void*)simgrid::mc::mmu::join(pageno, (std::uintptr_t)this->memory_);
// }

inline std::size_t PageStore::get_ref(std::size_t pageno)
{
  return this->page_counts_[pageno];
}

inline std::size_t PageStore::size()
{
  return this->top_index_ - this->free_pages_.size();
}

inline std::size_t PageStore::capacity()
{
